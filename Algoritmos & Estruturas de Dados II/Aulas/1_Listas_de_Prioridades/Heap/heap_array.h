...
Heap *new_heap_from(struct data[], uint32_t priorities[], uint8_t size, uint8_t max_size);
...
uint8_t heap_insert(Heap *, struct data, uint32_t priority);
...
Erros: `NULL_ERROR`, `NOT_OK` (se a heap estiver vazia), `OK`. */
uint8_t heap_pop(Heap *, struct data *);
