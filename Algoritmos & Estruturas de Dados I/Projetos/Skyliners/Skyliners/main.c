...
#define CLEAR system("pause"); system("cls");
#endif
...
- Jogando:
    ...
    2. Um rodada acaba quando todos tiverem jogado;
...
int main()
{
	...
	MEMORY_ERROR(board)
	_init_players(&players_pieces);

	// Game loop
	while (can_continue(players_pieces)) {
    ...
    int can_exit;
	// Menu de final de jogo, que permite ver as visões dos jogadores.
    do {
	...
	¯\_(ツ)_/¯  Mas, se fosse necessário, bastava fazer alguns loops e chamar as respectivas funções
...
int play(Board board, int player, List *player_pieces)
{
	...
	do {
		...
		input_alnum_coords(&line, &column, SIZE, SIZE, ">>> ",
			"Coordenada inválida!\nInsira uma letra seguida de um número correspondente às "
			"coordenadas do tabuleiro. Ex.: `B2`.\n");
...
void _init_players(List *(*players_pieces)[4])
{
	int i;
...
int can_continue(List *players_pieces[4])
{
	int i;
