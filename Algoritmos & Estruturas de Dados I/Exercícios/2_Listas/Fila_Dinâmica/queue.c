...
int add_to_queue(Queue *queue, struct student student)
{
	if (queue == NULL)
		return NULL_ERROR;
...
int queue_get(Queue *queue, struct student *student)
{
	CHECK_QUEUE(queue)
	*student = queue->start->data;
...
void empty_queue(Queue *queue)
{
	...
	queue->end = NULL;
	// free(queue); // Opcional: limpa o nó descritor da memória por completo.
	queue->size = 0;
}
...
int print_queue(Queue *queue)
{
	...
	while (aux != NULL) {
		printf("\t%d° aluno [%d]: %s.\n", i, aux->data.registration, aux->data.name);
		aux = aux->next;
		++i;
	}
